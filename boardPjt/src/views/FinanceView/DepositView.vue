    ...
    <table>
      ...
      <tbody>
        <tr v-for="product in filteredProducts" :key="product.deposit_code">
          ...
          <td><RouterLink
          :to="{ name: 'DepositDetailView', params: {id: product.deposit_code}}" class="custom-link">{{ product.fin_prdt_nm }}</RouterLink></td>
...
export default {
  ...
  computed: {
    filteredProducts() {
      return this.depositProducts.filter(product => {
